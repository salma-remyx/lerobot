...
### Autoregressive latent rollout (SG-JEPA)

By default the world model is trained with a single teacher-forced step: the
predictor sees the ground-truth context latents and predicts the next frame's
latent. `world_model_rollout_steps` turns on an *opt-in* multi-step objective
adapted from [Semigroup-JEPA: Latent Dynamics Consistency for Zero-Shot Physics
Generalization](https://arxiv.org/abs/2609.10464). When set to `k > 0`, the
one-step predictor is composed `k` times on its own predicted latents — each
prediction is re-fed as the newest context frame — and each rolled-out latent is
matched (L1) against the corresponding `k`-step-ahead encoder latent. Because
gradients flow through the whole rollout chain (predictions are not detached
between steps), the model is trained against the error accumulation that a
single-step loss never sees ("semigroup consistency": applying the dynamics
operator `k` times equals the `k`-step operator).

The rollout term is added on top of the single-step world-model loss and weighted
by `world_model_rollout_loss_weight`. The requested step count is clamped to the
available encoder context length. This feature is disabled by default
(`world_model_rollout_steps=0`), so it does not change the behaviour of the
...
lerobot-train \
  ...
  --policy.world_model_rollout_steps=3 \
  --policy.world_model_rollout_loss_weight=1.0 \
...
| `world_model_rollout_steps`  | 0             | SG-JEPA autoregressive latent-rollout steps. `0` (default) keeps only the single-step teacher-forced world-model loss. When `>0`, the one-step predictor is composed this many times on its own predicted latents and matched against the k-step-ahead encoder latents, adding a multi-step rollout loss on top of the single-step term. Clamped to the available encoder context length. See [Autoregressive latent rollout](#autoregressive-latent-rollout-sg-jepa) |
| `world_model_rollout_loss_weight` | 1.0     | Weight of the autoregressive rollout loss relative to the action loss. Only used when `world_model_rollout_steps > 0`                                                                                                                                       |
...
| `gripper_threshold`          | 0.5           | Threshold used by `pre_snap_gripper_action` and `binarize_gripper_action`. Note `binarize` runs _after_ unnormalization, so this is compared against the gripper's physical value                                                                          |
...
_normalized_ space, the unnormalizer maps those to the midpoint and the max, and `binarize` then
...
This view count used to be read from `jepa_tubelet_size`, which also names the JEPA encoder's _temporal_ tubelet size. `world_model_num_views` is the field for it now; leaving it at `None` falls back to `jepa_tubelet_size` so the published checkpoints keep loading unchanged.
