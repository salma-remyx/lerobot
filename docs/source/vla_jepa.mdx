...
### World-model feature alignment (optional)

`enable_feature_alignment` adds an opt-in, training-only distillation term that teaches the policy's embodied-action tokens to agree with the frozen V-JEPA world model's *internal* features (a `WorldModelFeatureAligner` projector scored by cosine distance), rather than by rolling the world model forward. The projector is used only during training and is never part of the inference path, so the deployed policy is identical to the baseline and any gain comes purely from the representation.

Because the frozen V-JEPA encoder supplies the target features, this requires `enable_world_model=True`. Setting `enable_feature_alignment=True` while the world model is disabled (including implicitly via `freeze_qwen=True`) logs a warning and turns feature alignment back off. The loss is scaled by `feature_alignment_loss_weight` and the projector's hidden width is set by `feature_alignment_hidden_dim`.

```bash
lerobot-train \
  --policy.path=lerobot/VLA-JEPA-Pretrain \
  --policy.enable_world_model=true \
  --policy.enable_feature_alignment=true \
  --policy.repo_id=your_org/your_repo \
  --dataset.repo_id=your_org/your_dataset
```

It defaults to `False`, so existing runs and the published checkpoints are unaffected.
...
| `enable_feature_alignment`   | `False`       | Add the training-only world-model feature-alignment loss (see [World-model feature alignment](#world-model-feature-alignment-optional)). Requires `enable_world_model=True`; if the world model is off it is disabled with a warning                        |
| `feature_alignment_loss_weight` | 1.0        | Weight of the feature-alignment loss relative to the action loss                                                                                                                                                                                          |
| `feature_alignment_hidden_dim`  | 1024       | Width of the alignment projector's hidden layer. The projector is training-only and never used at inference                                                                                                                                               |
