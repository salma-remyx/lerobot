...
- **`policy_parameters_push_frequency`** (`policy.actor_learner_config.policy_parameters_push_frequency`) – interval in _seconds_ between two weight pushes from the learner to the actor. The default is `4 s`. With adaptive synchronization enabled (see below) this acts as the **upper bound** on the interval between pushes, so actor staleness is still capped by this value. Decrease to **1-2 s** to provide fresher weights (at the cost of more network traffic); increase only if your connection is slow, as this will reduce sample efficiency.
- **`policy_sync_divergence_threshold`** (`policy.actor_learner_config.policy_sync_divergence_threshold`) – relative-L2 policy movement that triggers an *early* weight push instead of waiting for the fixed cadence. When the policy diverges from the last-pushed weights by at least this fraction, the learner refreshes the actors immediately; when it barely moves, redundant pushes are skipped. The default is `0.0`, which preserves the original fixed-cadence behavior. Set a small positive value (e.g. `0.05`) to make synchronization divergence-driven: fresher weights while learning is fast, less traffic once it plateaus.
- **`policy_parameters_push_min_frequency`** (`policy.actor_learner_config.policy_parameters_push_min_frequency`) – lower bound in _seconds_ between two pushes (and between divergence checks), clamped to `[0, policy_parameters_push_frequency]`. This rate-limits the (cheap) divergence computation so it never runs on every optimization step. The default is `0.0`. Only relevant when `policy_sync_divergence_threshold` is greater than `0.0`.
