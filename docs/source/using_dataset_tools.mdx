...
3. **Filter Unreliable Episodes** - Audit supervision quality and auto-drop low-quality episodes into a new dataset
...
7. **Modify Tasks** - Change the natural-language task descriptions associated with episodes
8. **Convert to Video** - Convert image-based datasets to video format for efficient storage (RGB and depth cameras are encoded with separate encoders)
9. **Re-encode Videos** - Re-encode an existing video dataset's RGB and/or depth streams with new encoder settings
10. **Show the Info of Datasets** - Show the summary of datasets information such as number of episode etc.
...
#### Filter Unreliable Episodes

Audit an existing dataset's supervision quality and automatically drop the least
reliable episodes into a new dataset. Each episode is scored by how far its
action/state supervision diverges from the rest of the corpus, and an automatic
outlier rule (a robust z-score cutoff, bounded by `max_drop_fraction`) selects
which episodes to remove via `delete_episodes`. When no episode is flagged the
source dataset is returned unchanged, so the operation is safe to run on a clean
dataset.

This tool is exposed through the Python API (`lerobot.datasets.filter_unreliable_episodes`)
rather than `lerobot-edit-dataset`:

```python
from lerobot.datasets import LeRobotDataset, filter_unreliable_episodes

dataset = LeRobotDataset("lerobot/pusht")

# Drop episodes whose supervision is a >3-sigma outlier, saving to a new dataset
curated = filter_unreliable_episodes(
    dataset,
    n_sigma=3.0,
    max_drop_fraction=0.5,
    repo_id="lerobot/pusht_curated",
)
```

**Parameters:**

- `feature_keys`: Numeric vector features to audit (defaults to the action and
  observation-state features present in the dataset stats).
- `n_sigma`: Robust-z cutoff above the median for flagging an episode (default: `3.0`).
- `max_drop_fraction`: Upper bound on the fraction of episodes that can be dropped
  in a single run, so a run can never prune away a healthy dataset (default: `0.5`).
- `output_dir` / `repo_id`: Location and identifier for the curated dataset (see `delete_episodes`).

> [!NOTE]
> The source dataset must have finalized aggregate statistics (call `finalize()` or
> recompute stats first); scoring compares each episode's per-feature statistics against
> the corpus-level `meta.stats`.
