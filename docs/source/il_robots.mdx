...
`lerobot-record` paces the loop at `--dataset.fps` and reports what it actually achieved: a one-line digest for every episode you keep, and a block for the whole session when recording ends — including on `Ctrl-C`.
...
This is worth reading before you train on the dataset. A frame's timestamp is derived from its index, so a session that ran at 25 Hz still produces a dataset that _claims_ 30 — the recorded motion is simply faster than reality. Above, `observe` is 74% of the loop's work and its `worst` is 40 ms on a 33.3 ms budget: the cameras are what pushed those 16 ticks over. **Pacing headroom** near zero means there is nothing left to absorb a slow tick.

The reset phases between episodes are paced the same way but are deliberately not part of these numbers, since they record nothing. `lerobot-teleoperate` and `lerobot-replay` report the same way, and `lerobot-rollout` adds the interpolation-specific lines described in [Cadence Reporting](./inference#cadence-reporting).
...
#### Capacity-aware policy report

Pass `--capacity_report=true` to add a capacity-aware report to the training banner (printed once at startup). Adapted from MINERVA (["How Small Can a Manipulation Policy Be and Still Solve LIBERO?"](https://arxiv.org/abs/2609.03715), arXiv:2609.03715), it reads the policy's `named_parameters()` and reports where the model sits relative to the empirically-measured LIBERO capacity bands, plus how its parameters split across the vision/action components the paper flags as the dominant levers:
...
```
MINERVA capacity band: efficient (0.54M learnable params -- within MINERVA's efficient regime (~0.25M-1.00M); the 0.54M reference hit 95.1% on LIBERO)
Capacity split: vision=0.30M (56%), action=0.18M (33%), other=0.06M (11%)
```

The report is disabled by default and never trains, prunes, or distills the policy — it only makes the capacity picture explicit before you tune the backbone or action-chunk length.
